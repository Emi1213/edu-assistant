...
export const API_ROUTES = {
  AUTH: {
    ...
    GOOGLE_LOGIN: `${AR_KEYS.AUTH}/google`,
    EXCHANGE: `${AR_KEYS.AUTH}/exchange`,
  ...
  LEARNING_OBJECTS: {
    ...
    CREATE_CONCEPT: (learningObjectId: number) =>
      `${AR_KEYS.LEARNING_OBJECTS}/${learningObjectId}/concepts`,
    NOTES: {
    ...
    ACTIVITIES: {
      LIST: (learningObjectId: number) =>
        `${AR_KEYS.LEARNING_OBJECTS}/${learningObjectId}/activities`,
      CREATE: (learningObjectId: number) =>
        `${AR_KEYS.LEARNING_OBJECTS}/${learningObjectId}/activities`,
      BY_ID: (learningObjectId: number, activityId: number) =>
        `${AR_KEYS.LEARNING_OBJECTS}/${learningObjectId}/activities/${activityId}`,
    },
    SESSIONS: {
      CREATE_OR_GET: (learningObjectId: number) =>
        `${AR_KEYS.LEARNING_OBJECTS}/${learningObjectId}/sessions`,
    },
  },
  CHAT: {
    SESSIONS: {
  ...
  TEACHER_FEEDBACK: {
    ...
    GET_BY_ID: (moduleId: number, id: number) =>
      `${AR_KEYS.MODULES}/${moduleId}${AR_KEYS.TEACHER_FEEDBACK}/${id}`,
    GENERATE: (moduleId: number) =>
      `${AR_KEYS.MODULES}/${moduleId}${AR_KEYS.TEACHER_FEEDBACK}/generate`,
  },
  STUDENT_FEEDBACK: {
    LIST: (moduleId: number) => `${AR_KEYS.MODULES}/${moduleId}${AR_KEYS.STUDENT_FEEDBACK}`,
    GET_BY_ID: (moduleId: number, id: number) =>
      `${AR_KEYS.MODULES}/${moduleId}${AR_KEYS.STUDENT_FEEDBACK}/${id}`,
    GENERATE: (moduleId: number) =>
      `${AR_KEYS.MODULES}/${moduleId}${AR_KEYS.STUDENT_FEEDBACK}/generate`,
  },
}
