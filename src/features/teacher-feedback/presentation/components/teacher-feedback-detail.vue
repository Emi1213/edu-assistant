...
import type { TeacherFeedback, AiFeedbackItem } from '../../types/teacher-feedback.types'
...
const getPriorityColor = (priority: AiFeedbackItem['priority']) => {
  switch (priority) {
...
const getPriorityLabel = (priority: AiFeedbackItem['priority']) => {
  switch (priority) {
  ...
  <div v-if="feedback" class="h-full flex flex-col bg-background">
    ...
    <div class="p-6 border-b border-border bg-muted/30">
      <div class="flex flex-wrap items-center gap-2 mb-4">
        <div v-if="isModuleScope" class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
          <BookOpen class="size-4 mr-2" />
          Módulo Completo
        </div>
        <div v-else class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-700">
      ...
      <div class="p-6 space-y-8 pb-12">
        ...
        <section class="space-y-3">
          <div class="flex items-center gap-2 text-primary">
            <Info class="size-5" />
            <h3 class="font-bold text-lg">Resumen Ejecutivo</h3>
          </div>
          <div class="bg-primary/5 border border-primary/10 rounded-xl p-5 text-foreground leading-relaxed">
        ...
        <section v-if="feedback.content.strengths?.length" class="space-y-4">
          <div class="flex items-center gap-2 text-green-600">
            ...
            <h3 class="font-bold text-lg">Fortalezas Identificadas</h3>
          </div>
          <div class="grid gap-4">
            ...
            >
              <div class="flex items-start justify-between gap-3 mb-2">
                <h4 class="font-bold text-foreground">{{ item.topic }}</h4>
              ...
              <p class="text-sm text-muted-foreground leading-relaxed">{{ item.detail }}</p>
            </div>
          </div>
        </section>
        ...
        <section v-if="feedback.content.improvements?.length" class="space-y-4">
          <div class="flex items-center gap-2 text-amber-600">
            ...
            <h3 class="font-bold text-lg">Áreas de Mejora</h3>
          </div>
          <div class="grid gap-4">
            <div 
        ...
        <section v-if="feedback.content.recommendations?.length" class="space-y-4">
          <div class="flex items-center gap-2 text-blue-600">
            ...
            <h3 class="font-bold text-lg">Recomendaciones Accionables</h3>
          </div>
          <div class="grid gap-4">
            <div 
            ...
            >
              <div class="flex items-start justify-between gap-3 mb-2">
                <h4 class="font-bold text-blue-900">{{ item.topic }}</h4>
                <div :class="[getPriorityColor(item.priority), 'px-2 py-0.5 rounded border text-[10px] uppercase tracking-wider font-bold shrink-0']">
                  {{ getPriorityLabel(item.priority) }}
                </div>
              </div>
              <p class="text-sm text-blue-800/80 leading-relaxed">{{ item.detail }}</p>
            </div>
          </div>
        </section>
      </div>
